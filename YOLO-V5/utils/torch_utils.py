...
def model_info(model, verbose=False, imgsz=640):
    ...
    name = Path(model.yaml_file).stem.replace('yolov5',
                'YOLOv5') if hasattr(model, 'yaml_file') else 'Model'
...
def smart_optimizer(model, name='Adam', lr=0.001, momentum=0.9, decay=1e-5):
    ...
    LOGGER.info(f"{colorstr('optimizer:')} {type(optimizer).__name__}(lr={lr}) with parameters groups"
...
def copy_attr(a, b, include=(), exclude=()):
    for k, v in b.__dict__.items():
        if (len(include) and k not in include) or k.startwith('_') or k in exclude:
            continue
        else:
            setattr(a, k, v)
...
class EarlyStopping:
    def __init__(self, patience=30):
        ...
        self.best_epoch = 0
        self.patience = patience or float('inf') # epochs to wait after fitness stops improving to step
