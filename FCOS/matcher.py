...
def get_ious_and_iou_loss(inputs,
    ...
    :param inputs: predict values
    :param targets: target values
    :param weight:  loss weight
    :param box_mode: 'xxyy' ==> x1, y1, x2, y2; 'ltrb'==> center2left, center2top, center2right, center2bottom
    :param loss_type: 'giou' or 'iou' or 'linear_iou'
    :param reduction: reduction manner
    :return: loss(tensor)
    """
    ...
    if loss_type == 'iou':
        ...
        giou = ious - (ac_uion - area_union) / ac_uion.clamp(min=eps)
        loss = 1 - giou
    else:
        raise NotImplementedError
...
class Matcher(object):
    def __init__(self,
        ...
        self.center_sampler_radius = cfg['center_sampling_radius']
        self.object_sizes_of_interest = cfg['object_sizes_of_interest']
        self.box_weights = box_weights

    def get_deltas(self, anchors, boxes):
        """
        Get box regression transformation deltas (dl, dt, dr, db) that
        can be used to transform the 'anchor' into the 'boxes'. That is,
        the relation "boxes = self.apply_deltas(deltas, anchors)" is true.

        :param anchors: Tensor, feature map coordinates
        :param boxes: target of the transformation, ground-truth
        :return: delta
        """
    ...
    def __call__(self, fpn_strides, anchors, targets):
        """
        :param fpn_strides: list[8, 16, 32,...] stride of network output.
        :param anchors: list of tensor list[F, M, 2], F = num_fpn_levels
        :param targets: Dict dict{'boxes': [...], 'labels': [...], 'orig_size': [...]}
        :return:
        """
        gt_classes = []
        gt_anchors_deltas = []
        ...
        for targets_per_image in targets:
            object_sizes_of_interest = [anchors_i.new_tensor(
                scale_range).unsqueeze(0).expand(anchors_i.size(0), -1)
                                        for anchors_i, scale_range in
                                        zip(anchors, self.object_sizes_of_interest)]
            ...
            if has_gt:
                ...
                if self.center_sampler_radius > 0:
                    ...
                    # no center sampling, it will use all the location within a ground-truth box
                    # [N, M], M = M1 + M2 + ... + MF
