...
def refine_targets(target, min_box_size):
    ...
    if len(target_bboxes) > 0:
        target_bboxes_wh = target_bboxes[..., 2:]
...
def mosaic_augment(img_list, tg_list, img_size):
    ...
    for i in range(len(img_list)):
        img_i, tg_i = img_list[i], tg_list[i]
        bboxes_i = tg_i["boxes"]
        labels_i = tg_i["labels"]
        ...
        if len(bboxes_i_) > 0:
            bboxes_i_[:, 0] = w * bboxes_i[:, 0] / w0 + pad_w
            bboxes_i_[:, 1] = h * bboxes_i[:, 1] / h0 + pad_h
            bboxes_i_[:, 2] = w * bboxes_i[:, 2] / w0 + pad_w
            bboxes_i_[:, 3] = h * bboxes_i[:, 3] / h0 + pad_h

            # bboxes_i_[:, 0] = (w * bboxes_i[:, 0] + pad_w) / w
            # bboxes_i_[:, 1] = (h * bboxes_i[:, 1] + pad_h) / h
            # bboxes_i_[:, 2] = (w * bboxes_i[:, 2] + pad_w) / w
            # bboxes_i_[:, 3] = (h * bboxes_i[:, 3] + pad_h) / h
...
class DistortTransform(object):
    ...
    def __init__(self, hue=0.1, saturation=1.5, exposure=1.5):
        super(DistortTransform, self).__init__()
        self.hue = hue
    ...
    def __call__(self, image: np.ndarray, target=None):
        """
        :param image: ndarray of shape HxW, HxWxC, or NxHxWxC. The array can be of type
                      uint8 in range [0, 255], or floating point in range [0, 1] or [0, 255]
        :param target: bbox array
        :return: ndarray, the distorted image
        """
...
class JitterCrop(object):
    ...
    def crop(self, image, pleft, ptop, output_size):
        orig_h, orig_w = image.shape[:2]
        src_w, src_h = output_size
        src_rect = [pleft, ptop, pleft + src_w, ptop + src_h]
        img_rect = [0, 0, orig_w, orig_h]
        ...
        dis_rect = [max(0, -pleft),
                    max(0, -ptop),
                    max(0, -pleft) + new_src_rect[2] - new_src_rect[0],
                    max(0, -ptop) + new_src_rect[3] - new_src_rect[1]]

        # crop the image
        cropped = np.zeros([src_h, src_w, 3], dtype=image.dtype)
        cropped[:, :, :] = np.mean(image, axis=(0, 1))
        cropped[dis_rect[1]:dis_rect[3],
                dis_rect[0]:dis_rect[2], :] = image[new_src_rect[1]:new_src_rect[3],
    ...
    def __call__(self, image, target=None):
        orig_h, orig_w = image.shape[:2]
        dis_w = int(orig_w * self.jitter_ratio)
        dis_h = int(orig_h * self.jitter_ratio)
        pleft = np.random.randint(-dis_w, dis_w)
        pright = np.random.randint(-dis_w, dis_w)
        ptop = np.random.randint(-dis_h, dis_h)
        pbot = np.random.randint(-dis_h, dis_h)

        swidth = orig_w - pleft - pright
        sheight = orig_h - ptop - pbot
...
class RandomShift(object):
    ...
    def __call__(self, image, target=None):
        if random.random() < self.p:
            ...
            new_image[new_y:new_y + new_h,
                      new_x:new_x + new_w, :] = image[
                                            orig_y:orig_y + new_h,
                                            orig_x:orig_x + new_w, :]
...
class ValTransforms(object):
    def __init__(self,
        ...
        self.transforms = Compose([
            ...
            Normalize(pixel_mean=pixel_mean, pixel_std=pixel_std)
        ])
